...
# offline catalog derive (pd-1uem) and the transform tier's nightly run
# (pd-8m5c) — an instance that is gone must not leave a timer behind still
# firing at its volume, its bucket and its lake network.
