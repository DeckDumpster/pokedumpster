...
pkdump_store_containers_conf() {
    ...
    cat > "$conf" <<EOF
# Generated by deploy/store-lib.sh — do not edit.
#
# tmp_dir keeps this store's libpod tmp out of the one every other store shares
# (pd-3zjt). On podman 4.9 that was also what split the rootless-netns
# scaffolding; on 5.x podman builds the netns under the store's own RUNROOT
# already, so the split no longer depends on this line. It is kept because
# Engine.TmpDir still holds other per-store state and --root/--runroot/--tmpdir
# still do not move it.
[engine]
tmp_dir = "${tmp_dir}"

# SLIRP4NETNS, NOT PASTA — and this is about AppArmor, not about networking.
#
# podman 5.x defaults to pasta, and Ubuntu ships pasta under an AppArmor profile
# (/etc/apparmor.d/usr.bin.pasta) that grants:
#
#     /tmp/**             rw,    # ... pasta_open_ns()
#     owner @{HOME}/**    w,     # pcap(), pidfile_open(), pidfile_write()
#
# pasta_open_ns() needs READ on the netns file podman hands it. Under /tmp it
# has rw; under \$HOME it has write only, so a store anywhere under \$HOME fails at
#
#     Couldn't open network namespace <runroot>/networks/rootless-netns/…:
#     Permission denied
#
# and podman reports the empty pid file as
#
#     rootless netns: kill network process: strconv.Atoi: parsing "": invalid syntax
#
# naming neither pasta nor AppArmor. Worse, when pasta DOES start, the profile
# declares no \`signal\` rules at all, so podman cannot signal the process it just
# launched and cleanup fails with
#
#     rootless netns: kill network process: permission denied
#
# Both were reproduced against podman 5.7.0 + passt 0.0~git20260120 on Ubuntu
# 26.04, and both are why this mechanism — whose entire purpose is running
# podman against a NON-DEFAULT --root/--runroot — cannot use pasta. The default
# store escapes it only because /run/user/\$UID is not \$HOME.
#
# slirp4netns carries no such profile, is still shipped and supported, and was
# measured bringing the netns up under this store's own runroot with the shared
# directory never created. Scoped to this file's stores; nothing changes for the
# default store or for anything not using store-lib.sh.
[network]
