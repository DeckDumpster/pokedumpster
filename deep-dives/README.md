...
| [`gate-suite-timing/`](gate-suite-timing/) | How long does `cargo test` + `cargo clippy` + `cargo fmt` take with a warm cargo cache, to size SPIRA_GATE_LOCK_WAIT? | **Done** — bead `db-pag5`; warm suite 97–181s (load-dependent), far below 360s lock wait. Current gate is `cargo fmt --check` (1–2s) after cheapgate fix. |
