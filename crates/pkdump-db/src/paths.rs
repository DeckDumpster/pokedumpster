...
//! ```
//!
//! Two path functions, and the difference between them is the whole point of
//! `pd-fci1`:
//!
...
//! * [`tenant_db_path_for_id`] takes a *`database_id`* — an opaque ULID the
//!   registry minted. Nothing outside this process ever chose one, so no
//!   caller-supplied string reaches a path constructor.
//!
...
///
/// Belt and braces. A `database_id` is generated by [`crate::registry::insert`]
/// and read back out of a `UNIQUE` column, so in a healthy system it is always
/// well-formed — but it is also the one string that *does* become a filename,
/// and the epic exists because a string that becomes a filename had only a
/// regex between it and the filesystem. Checked here rather than trusted:
/// the alphabet contains no `.`, `/` or `\`, so a value that passes cannot
/// name anything but a sibling inside `tenants/`.
///
...
/// Path to the collection database named by an opaque `database_id`.
///
...
pub fn tenant_db_path_for_id(database_id: &str) -> Result<PathBuf> {
    ...
    Ok(tenants_dir()?.join(format!("{database_id}.sqlite")))
}
